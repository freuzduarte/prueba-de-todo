    ...
    post {
        always {
            script {
                ...
                slackSend(channel: '#actividad-grupal-jenkinsfile', message: " Haciendo Pruebas con Sonar *${currentBuild.currentResult}:* build ${env.BUILD_NUMBER}, ${env.JOB_NAME} PROBLEMAS ${qg.status}", color: '#00FF04', )
            }
        }
    }
}
